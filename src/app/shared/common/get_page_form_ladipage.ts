...
$sections.each(function (i, v) {
  ...
  if ($(v).attr('lp-popup')) {
    ...
    section.rect.dimension.height = $container.css('height') ? Math.round(parseInt($(v).css('height').replace('px', ''), 10)) : 0;
    section.rect.dimension.width = $container.css('width') ? Math.round(parseInt($(v).css('width').replace('px', ''), 10)) : 0;
